...
ssr2::LowerBody::LowerBody(
    ...
    : _dir1(cos(PI / 2), sin(PI / 2)), _dir2(cos(PI * 7 / 6), sin(PI * 7 / 6)),
      _dir3(cos(PI * 11 / 6), sin(PI * 11 / 6)), _motor1(dir1, pwm1), _motor2(dir2, pwm2),
      _motor3(dir3, pwm3), _moving_average1(movAveCapacity), _moving_average2(movAveCapacity),
...
void ssr2::LowerBody::_setPowers_raw(float v1, float v2, float v3) {
    ...
    char buffer[256] = "";
    snprintf_P(
        buffer, 255, PSTR("[ssr2::LowerBody] set motor powers as %4d, %4d, %4d\n"), p1, p2, p3);
    Serial.print(buffer);
#endif /* ssr2_verbose */
}
